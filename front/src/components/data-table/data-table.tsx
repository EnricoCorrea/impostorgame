...
export function DataTable<T>({ rows, columns, rowKey, filters, onFilterChange, actions, loading, connected = true, emptyMessage, page, lastPage, onPageChange }:
  ...
  return <section className="table-card">
    <div className="table-heading"><div><span className="eyebrow">Componente complexo</span><h2>Dados e ações</h2></div><span className={`live-indicator ${connected ? "" : "offline"}`}><i /> {connected ? "API conectada" : "Falha na consulta"}</span></div>
