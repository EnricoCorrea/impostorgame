...
export function DataTable<T>({ rows, columns, rowKey, filters, onFilterChange, actions, loading, connected = true, emptyMessage, page, lastPage, onPageChange }:
  ...
  return <section className="table-card">
    <div className="table-heading"><div><h2>Dados e acoes</h2></div><span className={`live-indicator ${connected ? "" : "offline"}`}><i /> {connected ? "API conectada" : "Falha na consulta"}</span></div>
    ...
    <div className="table-scroll"><table><thead><tr>{columns.map((column) => <th key={column.key}>{column.label}</th>)}{actions && <th>Acoes</th>}</tr></thead>
      <tbody>{loading ? <tr><td colSpan={columnCount} className="empty">Carregando...</td></tr> : rows.length === 0 ? <tr><td colSpan={columnCount} className="empty">{emptyMessage}</td></tr> : rows.map((row) => <tr key={rowKey(row)}>{columns.map((column) => <td key={column.key}>{column.render(row)}</td>)}{actions && <td><div className="row-actions">{actions(row)}</div></td>}</tr>)}</tbody>
    </table></div>
    <footer className="pagination"><span>Pagina {page} de {Math.max(lastPage, 1)}</span><div><ActionButton variant="ghost" disabled={page <= 1} onClick={() => onPageChange(page - 1)}>Anterior</ActionButton><ActionButton variant="ghost" disabled={page >= lastPage} onClick={() => onPageChange(page + 1)}>Proxima</ActionButton></div></footer>
  </section>;
}
